        ...
        <LinearGradient colors={['#2C5D63', '#438A94']} style={styles.safeArea}>
            <SafeAreaView style={{ flex: 1 }}>
                ...
                <ScrollView contentContainerStyle={styles.gridContainer}>
                    {sounds.map(sound => (
                        <TouchableOpacity key={sound.id} style={[styles.card, { height: sound.height }, sound.cardStyle]} onPress={() => router.push('/(main)/play')}>
                            <Text style={styles.cardTitle}>{sound.title}</Text>
                            <ImageBackground source={sound.image} style={[styles.cardImage, { width: sound.imageWidth, height: sound.imageHeight }, sound.imageStyle]} imageStyle={{ borderRadius: 15 }}>
                            ...
                            {sound.subtitle && <Text style={styles.cardSubtitle}>{sound.subtitle}</Text>}
                        </TouchableOpacity>
                    ))}
                </ScrollView>
            </SafeAreaView>
...
export default SleepSoundScreen;
